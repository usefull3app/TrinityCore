    ...
    public:
        ...
        static bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            if (sTransmogrification->EnableTransmogInfo)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|t关于幻化", EQUIPMENT_SLOT_END + 9, 0);
            ...
            if (sTransmogrification->EnableSets)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/RAIDFRAME/UI-RAIDFRAME-MAINASSIST:30:30:-18:0|t管理幻化集", EQUIPMENT_SLOT_END + 4, 0);
#endif
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|t清除所有的幻化效果", EQUIPMENT_SLOT_END + 2, 0, "你确定要清理所有装备的幻化效果吗?", 0, false);
                ...
                case EQUIPMENT_SLOT_END + 4: // Presets menu
                    {
                        ...
                        if (sTransmogrification->EnableSetInfo)
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|t什么是幻化集", EQUIPMENT_SLOT_END + 10, 0);
                        ...
                        if (!player->presetMap.empty())
                        {
                            ...
                            if (player->presetMap.size() < sTransmogrification->MaxSets)
                                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|t保存为幻化集", EQUIPMENT_SLOT_END + 8, 0);
                        }
                        else
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|t保存为幻化集", EQUIPMENT_SLOT_END + 8, 0);
                ...
                case EQUIPMENT_SLOT_END + 6: // view preset
                    {
                        ...
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|t使用该幻化集", EQUIPMENT_SLOT_END + 5, action, "使用该幻化集进行幻化将会使被幻化的装备与你绑定, 是否断续使用?\n\n" + it->second.name, 0, false);
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|t删除该幻化集", EQUIPMENT_SLOT_END + 7, action, "你确定要删除该装备幻化集?" + it->second.name + "?", 0, false);
                ...
                case EQUIPMENT_SLOT_END + 8: // Save preset
                    {
                        ...
                        if (canSave)
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|t保存幻化集", 0, 0, "你是否需要保存幻化集?", cost*sTransmogrification->SetCostModifier + sTransmogrification->SetCopperCost, true);
        ...
        static void ShowTransmogItems(Player* player, Creature* creature, uint8 slot) // Only checks bags while can use an item from anywhere in inventory
        {
            ...
            if (oldItem)
            {
                ...
                for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
                {
                    ...
                    if (oldItem->transmog == newItem->GetEntry())
                        continue;
                    ++limit;
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sTransmogrification->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sTransmogrification->GetItemLink(newItem, session), slot, newItem->GetGUID().GetCounter(), "使用该幻化集进行幻化将会使被幻化的装备与你绑定, 是否断续使用?\n\n" + sTransmogrification->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sTransmogrification->GetItemLink(newItem, session) + ss.str(), price, false);
                }
            ...
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|t清除幻化效果", EQUIPMENT_SLOT_END + 3, slot, "你是否要清除装备槽上的幻化效果?", 0, false);
